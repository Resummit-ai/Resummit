...
exports.generateBatchBullets = void 0;
exports.callAI = callAI;
exports.safeParseJSON = safeParseJSON;
exports.generateCVFromRepos = generateCVFromRepos;
exports.regenerateSummary = regenerateSummary;
exports.regenerateBullet = regenerateBullet;
exports.calculateATSScore = calculateATSScore;
exports.suggestSkills = suggestSkills;
exports.suggestSkillsFromGitHub = suggestSkillsFromGitHub;
// lib/aiService.ts
require("server-only");
...
let _redis = null;
function getBudgetRedis() {
    if (_redis)
        return _redis;
    if (REDIS_URL && REDIS_TOKEN && REDIS_URL.startsWith('https://')) {
        try {
            _redis = new redis_1.Redis({ url: REDIS_URL, token: REDIS_TOKEN });
        }
        catch {
            _redis = null;
        }
...
async function checkAIBudget(userId) {
    if (!userId)
        return true; // no user context → allow (background jobs)
    const redis = getBudgetRedis();
    if (!redis)
        return true; // Redis offline → allow
    const now = new Date();
    ...
    try {
        ...
        if (count > DAILY_AI_LIMIT) {
            logger_1.logger.ai({ userId, feature: 'budget.exceeded', model: GEMINI_MODEL, totalTokens: 0 });
            return false;
        }
    }
    catch {
 ...
 */
async function generateCVFromRepos(repos, targetRole = 'Software Engineer') {
...
${JSON.stringify(repoSummary, null, 2)}`;
    ...
    try {
        ...
        const allLanguages = Array.from(new Set(repos.map(r => r.language).filter(Boolean)));
        const allTopics = Array.from(new Set(repos.flatMap(r => r.topics || []).filter(Boolean)));
        parsed = {
    ...
    return {
        ...
        projects: (parsed.projects || []).slice(0, 3).map((p) => ({
...
exports.generateBatchBullets = generateCVFromRepos;
async function regenerateSummary(projects, skills, experience, targetRole, profileReadme) {
...
async function regenerateBullet(bullet, projectName, tech, targetRole) {
...
- Return only the bullet text, nothing else`;
    ...
    try {
        ...
        const techStr = Array.isArray(tech) ? tech.slice(0, 2).join(', ') : tech;
...
function calculateHeuristicScore(cvText, targetRole) {
...
async function calculateATSScore(cvText, targetRole) {
    ...
    try {
        ...
        return {
            ...
            breakdown: {
                ...
                overall: Math.min(100, Math.max(0, parseInt(parsed.score) || 50))
            },
...
async function suggestSkills(projects, existing) {
 ...
 */
async function suggestSkillsFromGitHub(repos, projects, existing) {
    ...
    const repoSummaries = repos.map(r => {
        const parts = [];
        if (r.name)
            parts.push(`Repo: ${r.name}`);
        if (r.language)
            parts.push(`Language: ${r.language}`);
        if (r.topics?.length)
            parts.push(`Topics: ${r.topics.join(", ")}`);
        if (r.description)
            parts.push(`Description: ${r.description}`);
        if (r.readme)
            parts.push(`README excerpt: ${r.readme.slice(0, 400)}`);
        return parts.join(" | ");
    }).join("\n");
...
${projects.slice(0, 5).map((p) => `${p.title || p.name}: ${(p.techStack || []).join(", ")}`).join("\n")}
...
Return ONLY valid JSON, no explanation:
{
  ...
  "tools": []
    ...
    try {
        ...
        const parsed = safeParseJSON(raw);
        return {
            languages: Array.isArray(parsed.languages) ? parsed.languages.filter((s) => typeof s === "string" && s.length > 0) : [],
            frameworks: Array.isArray(parsed.frameworks) ? parsed.frameworks.filter((s) => typeof s === "string" && s.length > 0) : [],
            tools: Array.isArray(parsed.tools) ? parsed.tools.filter((s) => typeof s === "string" && s.length > 0) : [],
        };
    }
    catch {
        return { languages: [], frameworks: [], tools: [] };
